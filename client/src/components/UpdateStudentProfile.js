...
const UpdateStudentProfile = ({ user, edit, UpdateStudentByAdmin }) => {
 // console.log(user);
  //validation
  const validation = Yup.object({
    ...
    student_group: Yup.string().required("this field is required"),
  });

  const formFieldID = () => {
    if (user.level == "Licence 1") {
      return "licence1";
    }
    if (user.level == "Licence 2") {
      return "licence2";
    }
    if (user.level == "Licence 3") {
      return "licence3";
    }
    if (user.level == "Master 1") {
      return "master1";
    }
    if (user.level == "Master 2") {
      return "master2";
    }
  };
    ...
    <Wrapper>
      <Content style={{ opacity: edit && 0.3, pointerEvents: edit && "none" }}>
        <Formik
          ...
          onSubmit={(values) => UpdateStudentByAdmin(values, user.id_user)}
        >
          {(formik) => (
            <div>
              <Form style={{ width: "100%" }}>
                ...
                <Shared>
                  <InputShared>
                    ...
                    <TextFieldOptions
                      options={formField[formFieldID()].departmentOptions}
                      name="department"
                    />
                  </InputShared>
                  <InputShared>
                    <Label>Level *</Label>
                    <TextFieldOptions
                      options={formField[formFieldID()].levelOption}
                      name="level"
                    />
                  </InputShared>
                </Shared>
                ...
                <Shared>
                  <InputShared>
                    ...
                    <TextFieldOptions
                      options={
                        formField[formFieldID()].section_specialityOption
                      }
